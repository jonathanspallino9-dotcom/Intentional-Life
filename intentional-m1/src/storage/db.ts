...
export class IntentionalDB extends Dexie {
 ...
 constructor(name='hollowMoon'){super(name);this.version(1).stores({activities:'id,archivedAt',activityVersions:'id,activityId,effectiveFrom',occurrences:'id,activityId,activityVersionId,effectiveStart,effectiveEnd,status,&[activityId+effectiveStart+effectiveEnd]',dayRecords:'id,&localDate',domains:'id,displayOrder',intentions:'id,dayId,domainId,&[dayId+domainId]',reflections:'id,&intentionId',productivityEvents:'id,type,sourceId,effectiveDate,&[type+sourceId]',settings:'id',metadata:'key'});}
}
